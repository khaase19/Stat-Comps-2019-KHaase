...
There are four main types of clustering: hierarchial, partitioning, density-based, and methods-based. We will now dive into the partioning clustering techinque. 
...
Typically, there is a dataset with _n_ observations and the goal is to divide them into  _K_ clusters so that an objective function is optimized (sum of square distances is minimized). 

The most common objective function is the sum of squared errors (SSE), where $c_k$ is the centroid or mediod of the cluster $C_k$.
